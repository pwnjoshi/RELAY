...
import { BatchCampaign, BatchFollowupItem, CallRecord } from "@/lib/types";
import fs from "fs";
...
export async function POST(req: Request) {
  try {
    ...
    const items: BatchFollowupItem[] = contacts.map((c: any, index: number) => ({
    ...
    return NextResponse.json({ ok: false, error: err.message || "Failed to create campaign" }, { status: 500 });
  }
}
