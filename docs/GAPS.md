...
| **Durable End-to-End Idempotent Dispatch & TTL Cache** | Phase 6 Requirement, Round 3 | **Fully Implemented** | [`components/TriggerModal.tsx#L330`](../components/TriggerModal.tsx#L330), [`app/batch/page.tsx#L210`](../app/batch/page.tsx#L210), [`app/api/batch/execute/route.ts#L22`](../app/api/batch/execute/route.ts#L22), [`lib/idempotency.ts`](../lib/idempotency.ts), [`lib/supabase.ts#L225`](../lib/supabase.ts#L225) | Single calls, recalls, and batch campaigns attach a client-side UUID `Idempotency-Key` header. Backend checks multi-tier cache (sliding-window 24h TTL in-memory eviction + Supabase `idempotency_keys` table) and returns cached response on re-submission without double-dialing. |
